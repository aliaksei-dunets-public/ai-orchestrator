...
version: 2.1.0
description: >-
  Perform model-led, evidence-based reviews of Python changes, components, and
  full projects. Reconstruct the system and its behavior before applying
  structured quality and Python-specific coverage checks. Trace end-to-end
  flows, architecture, state, failure propagation, and blast radius, then use
  an independent reviewer to challenge the primary analysis. Review only unless
...
- major features and release candidates;
- security-, data-, financial-, trading-, concurrency-, authentication-, or
  migration-sensitive work;
- database and public API changes;
- project audits;
- any review with a Critical or Blocking candidate finding.

It is recommended for other non-trivial work.
