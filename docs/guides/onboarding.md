...
После успешного применения следуйте шагам установки и проверки Task Manager выше. Во внешнем проекте добавленный блок `AGENTS.md` указывает агенту путь к Task Manager skill внутри `tools/orchestrator`; skill выбирает MCP stdio для внешнего агента, прямой Python API для in-process оркестратора и CLI как fallback. Graph Runtime и автоматическая обработка задач ещё не реализованы. Если preview сообщает о широком правиле `.orchestrator/` в `.gitignore`, согласуйте его замену на `.orchestrator/state/` и создайте новый план.
