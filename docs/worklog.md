...
- Добавлены `archive_task`, `unarchive_task` и `purge_task`; архив скрывается из списка по умолчанию, а физическое удаление разрешено только архивным `cancelled` задачам после трёх календарных месяцев с terminal-события.
