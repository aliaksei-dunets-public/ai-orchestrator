...
| Specification и Plan | Будущий слой артефактов; сейчас файлы готовит вызывающая сторона | `.orchestrator/tasks/TASK-xxxx/specification.md`, `plan.md` | Можно версионировать |
...
Task Manager не создаёт Specification и Plan. Вызывающая сторона готовит файлы, затем регистрирует их через `attach_artifact` с путём, ссылкой и SHA-256. Перед `ready` сервис повторно проверяет файлы и привязку одобрения и Execution Package к той же паре хешей.

В текущем срезе Plan Review и Execution Package представлены ссылками и метаданными внутри снимка SQLite. Отдельные файлы манифестов, полный репозиторий версий и проверка содержательных схем ещё не реализованы. Наличие хеша подтверждает неизменность файла, но не достаточность спецификации или качество плана.
