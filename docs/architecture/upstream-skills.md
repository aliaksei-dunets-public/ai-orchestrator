...
The upstream directories are vendored without semantic rewriting so their references, templates, evals and scripts remain internally consistent. Orchestrator-owned coordinator skills stay small and define routing, shared result schemas and workflow transitions.
...
Upstream updates require a new compatibility review, a recorded commit, contract tests and regeneration of the release artifact; they are never pulled into runtime automatically.
