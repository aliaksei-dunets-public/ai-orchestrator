...
`Node(node_id, input_contract, output_contract, outcomes, transitions, required_outputs=(), required_outputs_by_outcome={})` требует непустые строки контрактов и ID, уникальные непустые outcomes и переход для каждого outcome. Target — существующий узел либо терминальное состояние. Ссылки input/output_contract не являются встроенным реестром JSON-схем; семантику проверяет владелец контракта.

`required_outputs` перечисляет базовые обязательные ключи data/artifacts для всех исходов. `required_outputs_by_outcome` — mapping объявленных outcomes в списки обязательных ключей: запись полностью заменяет базовый список для конкретного исхода, пустой список явно снимает требования к outputs. Если записи нет, действует базовый список. Оба контейнера копируются; overrides нормализуются в tuple и неизменяемый mapping. Graph.from_dict принимает оба поля. Неизвестный outcome или malformed список overrides отклоняется при создании Node.

Например, базовый `("plan",)` и overrides `{"needs_input": (), "blocked": (), "failure": ("diagnostics",)}` позволяют спросить до готового плана; успех требует plan, ошибка требует diagnostics. Для доменных `approved`/`failed` требования задаются по точному имени; runtime не угадывает успешность по имени. Старый граф без overrides сохраняет прежнюю проверку всех исходов. Форма артефактов и остальные guards проверяются даже при пустом списке.
