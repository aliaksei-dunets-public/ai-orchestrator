...
Graph Runtime, Executor Loop, Artifact Repository, полноценные графовые переходы и синхронизация с внешними трекерами. Реализован только первый входной срез Request Router → Task Creator; полного устанавливаемого пакета всего ядра также пока нет.
...
Последний результат: корневой Orchestrator — 5 тестов; Onboarding — 15 тестов, 14 прошли и 1 пропущен из-за ограничения Windows на symlink; Task Manager — 30 тестов прошли. `orchestrator-tasks --project . validate` вернул `ok: true`. Оба пакета собираются в отдельные wheel; Task Manager проверен изолированно без ядра.
